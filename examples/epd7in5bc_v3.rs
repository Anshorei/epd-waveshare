...
use epd_waveshare::{
    color::TriColor,
    epd7in5bc_v3::{Display7in5bc, Epd7in5bc},
...
fn main() -> Result<(), std::io::Error> {
    ...
    let mut epd7in5 =
        Epd7in5bc::new(&mut spi, cs, busy, dc, rst, &mut delay).expect("eink initalize error");
    ...
    epd7in5.update_color_frame(&mut spi, display.bw_buffer(), display.chromatic_buffer()).unwrap();
    epd7in5.display_frame(&mut spi, &mut delay).unwrap();

    println!("Finished tests - going to sleep");
    epd7in5.clear_frame(&mut spi, &mut delay).unwrap();
    epd7in5.sleep(&mut spi, &mut delay)
}

fn draw_text(display: &mut Display7in5bc, text: &str, x: i32, y: i32) {
    let style = MonoTextStyleBuilder::new()
        .font(&embedded_graphics::mono_font::ascii::FONT_6X10)
        .text_color(TriColor::White)
